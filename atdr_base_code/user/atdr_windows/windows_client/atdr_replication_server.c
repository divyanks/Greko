...
void replic_server_obj_setup(struct atdr_replication *replic_obj, int server_pid)
{
	...
	replic_server_obj[server_pid].rep_conn = &atdr_conn_server[server_pid];
	replic_server_obj[server_pid].rep_hdr = &replic_hdr_server_obj[server_pid]; 
	replic_server_obj[server_pid].rep_disk = &atdr_disk_src_obj[ds_count];
	//  rs_count++;
}

void replic_server_obj_destroy(struct atdr_replication *replic_obj)
{
	free(replic_obj);
}

struct atdr_replication_operations replic_server_operations = 
{	
