...
int send_hdr(char *buf, int size, SOCKET sockfd, replic_header *rep_hdr_rcvd)
{
    ...
    if( ret <= 0)
    {
        ebdr_log(EBDR_ERROR, "ebdr conn client send hdr error...returning -1\n ");
		return -1;
...
int send_data_start(char *buf, int size, SOCKET sockfd, replic_header *rep_hdr_rcvd)
{
	...
	if( ret <= 0)
    {
        ebdr_log(EBDR_ERROR, "ebdr conn client send data start error return -1");
		return -1;
...
{
	ebdr_log(EBDR_INFO, "[rm_relationpeer] rid = %d\n", rid);
	if(rid > 0)
	{
		ebdr_log(EBDR_INFO, "[rm_relationpeer] calling CLIENT relation_obj_destroy \n");
...
int recv_hdr(char *buf, int size, SOCKET sockfd, replic_header *rep_hdr_rcvd)
{
  ...
  ebdr_log(EBDR_INFO, "[client_recv_hdr] receiving replic header \n");
  rep_hdr_rcvd = (replic_header *)buf;
  ebdr_log(EBDR_INFO, "[client_recv_hdr] opcode = %d\n", rep_hdr_rcvd->opcode);
 ret = recv(sockfd, (char *)rep_hdr_rcvd, sizeof(struct replication_header), 0); 

  if( ret <= 0)
  {
    ebdr_log(EBDR_ERROR, "ebdr conn client recv error");
    stop_work("[recv_hdr] conn client recv error ");
  }
  switch(rep_hdr_rcvd->opcode)
  {
    case SYN_ACK:
      ebdr_log(EBDR_INFO, "[client_recv_hdr] ========SYN_ACK recevied from server =====\n");	
      pid = rep_hdr_rcvd->partner_id;
      ebdr_log(EBDR_INFO, "[client_recv_hdr] partner_id = %d\n", pid);
    ...
    case RM_RELATION_PEER:
      ebdr_log(EBDR_INFO, "[client_recv_hdr] ====== RM_RELATION_PEER received from server =====\n");
...
//      replication_terminate(pid); santhosh
      break;
    default: 
      ebdr_log(EBDR_INFO, "illegal opcode\n");
      break;
  }
...
int verify_data_resp(int pid, replic_header * rep_hdr_rcvd)
{
  ...
  if(calculate_md5(ebdr_disk_target_obj[pid].name, rep_hdr_rcvd, md5) < 0) {
    ...
    ebdr_log(EBDR_FATAL, "error md5 calculation:");
    goto out;
  }
...
int recv_data(char *buf, int size, SOCKET sockfd, replic_header *rep_hdr_rcvd)
{
    ...
    if(ret <= 0)
    {
        ebdr_log(EBDR_INFO, "[client]recv error");
		stop_work("[recv_data] client recv error");
    }
    ebdr_log(EBDR_INFO, "[***Client_data_type] usr_bitmap_rcvd->bitmap_area= 0x%lx\n",*(unsigned long int *)buf);
    return 0;
}
...
int recv_data_resp(char *buf, int size, SOCKET sockfd, replic_header *rep_hdr_rcvd)
{
	...
	if(rep_hdr_rcvd->opcode == DATA_PREP)
	{
		...
		ebdr_log(EBDR_INFO, "client recieved DATA_PREP ############## size=%lu\n", rep_hdr_rcvd->available_size);
