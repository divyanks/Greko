...
static int bitmap_client_setup(struct ebdr_user_bitmap *ubitmap_obj, int client_pid)
{
	...
	ebdr_log(EBDR_INFO, "[bitmap_client_setup] client_pid = %d\n", client_pid);
 	return 0;
}
...
int load_bitmap_from_disk(struct ebdr_user_bitmap *ubitmap_obj, int pid)
{
	...
	if (fp == NULL)
	{
		ebdr_log(EBDR_ERROR, "Error while opening the bitmap file to Load.\n");
...
int store_bitmap_to_disk(struct ebdr_user_bitmap *ubitmap_obj, int pid)
{
	...
	if (fp == NULL)
	{
		ebdr_log(EBDR_ERROR, "Error while opening the bitmap file to write.\n");
...
{
	ebdr_log(EBDR_INFO, "[bitmap_client_destroy] freeing bitmap_area... \n");
