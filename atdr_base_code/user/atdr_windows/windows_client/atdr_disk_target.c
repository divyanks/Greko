...
void disk_target_create(char *name, struct ebdr_disk *ebdr_disk_obj)
{
  ...
  client_pid = dt_count;
	...
	//if(client_pid < MAX_DISKS && (ebdr_disk_target_obj[client_pid].obj_state != DISK_OBJ_IN_USE))
	{
		...
		ebdr_log(EBDR_INFO, "[target disk] disk name:%s\n", ebdr_disk_obj->name);
		if (recovery_mode[client_pid] != 1)
		{
			ebdr_disk_obj->bitmap_size = all_relation_clients[client_pid].bitmap_size;
			insert_into_disk(client_pid, ebdr_disk_obj->name, "-", ebdr_disk_obj->bitmap_size, ebdr_disk_obj->obj_state, "ebdrdbc"); 
		}
		ebdr_disk_obj->ops = &disk_target_ops;
	}
	else
	{
		ebdr_log(EBDR_ERROR, "[disk_target_create] MAX DISKS reached !! \n");
		//client_pid = 0;
	}
}
...
int disk_target_setup(struct ebdr_disk *disk_obj)
{
	...
	{
		ebdr_log(EBDR_ERROR, "INVALID_FILE SOURCE %d %s\n", GetLastError()); /* This print will change later */
	   return -1;
	}
	...
	ebdr_log(EBDR_INFO, "[disk] target fd:%d\n", disk_obj->disk_fd);
	dt_count++;
	ebdr_log(EBDR_INFO, "[disk_target_setup] dt_count = %d\n", dt_count);
	return 0;
}

void disk_target_destroy(struct ebdr_disk *disk_obj)
{
	ebdr_log(EBDR_INFO, "[disk_target_destroy] setting obj_state to DISK_OBJ_RELEASED \n");
	disk_obj->obj_state = DISK_OBJ_RELEASED;
}

int mkdisk_from_db_on_client(void)
{
	return retrieve_from_disk(ebdr_disk_target_obj);
}


void disk_target_unsetup(struct ebdr_disk *disk_obj)
{
	ebdr_log(EBDR_INFO, "[disk_target_unsetup] closing target disk_fd: %d\n", disk_obj->disk_fd);
