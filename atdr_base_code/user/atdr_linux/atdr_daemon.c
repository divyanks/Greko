...
__thread int logging;

pthread_mutex_t condition_mutex = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t  condition_cond  = PTHREAD_COND_INITIALIZER;

pthread_t resync_tid[MAX_DISKS];

__thread pthread_t log_pid;
...
void snapshot_disk_init(char *snap_disk, int pid)
{
	...
	ebdr_log(INFO, "[snapshot_disk_init] recovery_mode[%d] = %d\n", pid, recovery_mode[pid]);
...
void *resync_handler(void *partner_id)
{
  ...
  //Setting the thread specific pid
  	log_pid = pid;
...
static int read_from_fifo(int fifo_fd)
{
	int bytes;
...
static int open_fd_for_ioctl(void)
{
