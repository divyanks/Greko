...
int server_send_data(char *buf, int size, int sockfd, replic_header *rep_hdr_rcvd)
{
	...
	if( ret < 0)
	{
		ebdr_log(ERROR, "[data_type] ebdr conn server send data error returning -1\n");
		return -1;
...
int get_bitmap(int sockfd, replic_header *rep_hdr_rcvd)
{
	...
	replic_server_obj[pid].rep_conn->conn_ops->do_ebdr_conn_send(ebdr_disk_src_obj[pid].bitmap->bitmap_area, 
			ebdr_disk_src_obj[pid].bitmap->bitmap_size, all_partner_servers[pid].socket_fd, DATA_TYPE);
...
int get_data(int sockfd, replic_header *rep_hdr_rcvd)
{
	...
	if((io_server_obj[pid].ops->chunk_write(all_partner_servers[pid].socket_fd, rbuff, size, start_lba)) < 0)
	{
		...
		//ebdr_log(INFO, "[get_data] closing send_recv_thread ...\n");
