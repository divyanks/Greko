...
void replic_hdr_client_create(enum rep_hdr_role_t rep_hdr_role, int client_pid)
{
	...
	{
		ebdr_log(INFO, "Max replic header client objects reached !\n");
...
struct replication_header *replic_hdr_client_nego(struct replication_header *rep_hdr_obj, int sockfd, int client_pid)
{
	...
	ebdr_log(INFO, "[client_proto_nego] Agreed on these sizes bitmap:%lu grain:%lu chunksize=%lu client_pid: %d\n",
...
{
	ebdr_log(INFO, "[replic_hdr_client_destroy] setting obj_state to REP_HDR_OBJ_RELEASED\n");
