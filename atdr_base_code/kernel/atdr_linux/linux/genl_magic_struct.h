...
/*
 * @EBD_GENLA_F_MANDATORY: By default, netlink ignores attributes it does not
 * know about.  This flag can be set in nlattr->nla_type to indicate that this
 * attribute must not be ignored.
 *
 * We check and remove this flag in ebdr_nla_check_mandatory() before
 * validating the attribute types and lengths via nla_parse_nested().
 */
#define EBD_GENLA_F_MANDATORY (1 << 14)

/*
 * Flags specific to ebdr and not visible at the netlink layer, used in
 * <struct>_from_attrs and <struct>_to_skb:
 *
 * @EBD_F_REQUIRED: Attribute is required; a request without this attribute is
 * invalid.
 *
 * @EBD_F_SENSITIVE: Attribute includes sensitive information and must not be
 * included in unpriviledged get requests or broadcasts.
 *
 * @EBD_F_INVARIANT: Attribute is set when an object is initially created, but
 * cannot subsequently be changed.
 */
#define EBD_F_REQUIRED (1 << 0)
#define EBD_F_SENSITIVE (1 << 1)
#define EBD_F_INVARIANT (1 << 2)

#define __nla_type(x)	((__u16)((x) & NLA_TYPE_MASK & ~EBD_GENLA_F_MANDATORY))
...
#define __field(attr_nr, attr_flag, name, nla_type, type,	\
	...
	T_ ## name = (__u16)(attr_nr | ((attr_flag) & EBD_GENLA_F_MANDATORY)),
