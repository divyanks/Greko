...
*/
	...
	if (result == S_OK)
	{
		...
		if (result == S_OK)
		{
			...
			if (result != S_OK){ _tprintf(_T("Error: HRESULT = 0x%08lx\n"), result); ReleaseInterface(pBackup); return -19; }
			...
			if (result == S_OK)
			{
				...
				if (result == S_OK)
				{
					...
					if (result != S_OK){ _tprintf(_T("- Returned HRESULT = 0x%08lx\n"), result); ReleaseInterface(pBackup); }
					if (result == S_OK)
					{
										...
										if (result == S_OK)
										{
											...
											if (result == S_OK)
											{
												...
												if (result == S_OK)
												{
													...
													_tprintf (_T("\n"));
													wchar_t tmp[128];
													wcscpy(tmp, snapshotProp.m_pwszSnapshotDeviceObject);
													wcscat(tmp, L"\\");
													returnValue = CreateSymbolicLink(L"c:\\shadow", tmp, 1);
